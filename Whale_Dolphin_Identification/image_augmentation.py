...
train_transform = customized_augmentation(image_size, color_image, training_purpose)

You may use this file for visualization purpose:
...
def mean_std(color_image=False):
    ...
    if color_image == True:
        ...
        # grayscale image
        MEAN = (0.60623126, 0.60623126, 0.60623126)
        STD = (0.19362484, 0.19362484, 0.19362484)
...
def percentile_scaling(img_tensor):
    ...
    lower = np.percentile(img_array, 0.5)
    upper = np.percentile(img_array, 99.5)
...
def customized_augmentation(image_size, color_image=False, training_purpose=True):
    ...
    post_transforms = Compose([
        *([_convert_to_rgb] if not color_image else []),
