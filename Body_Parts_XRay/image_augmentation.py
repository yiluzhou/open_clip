...
def customized_augmentation(image_size, color_image=False):
    ...
    train_transform = Compose([
        ...
        RandomAugmentation(0.15, pre_rotation), # 20% images: RandomRotation
        pre_resize, # all images do resize
        RandomAugmentation(0.15, customize_RandomResizedCrop), # 20% images will have be resized here
        # 0.3 means 30% of images will be augmented
        RandomAugmentation(0.15, pre_transforms, AlbumentationsTransform(albu_transforms)),
        post_transforms
    ])
